<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Contacts.Loireatlantique.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<tfoot>
				<?php if ($this->auth->has_permission('Contacts.Loireatlantique.Delete')) : ?>
				<tr>
					<td colspan="6">
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('Contacts.Loireatlantique.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/loireatlantique/contacts/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->CIVILITE) ?></td>
				<?php else: ?>
