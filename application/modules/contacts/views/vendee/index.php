<div class="admin-box">
	<h3><?php echo lang('contacts_title');  ?>Vendée</h3>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Contacts.Vendee.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<?php foreach ($records as $record) : ?>
				...
				<td>
                                ...
                                <?php if ($this->auth->has_permission('Contacts.Vendee.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/vendee/contacts/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>Modifier') ?></td>
				<?php else: ?>
