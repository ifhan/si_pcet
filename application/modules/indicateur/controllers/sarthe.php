...
class sarthe extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Indicateur.Sarthe.View');
	...
	*/
	public function index()
	{
		...
		$records = $this->indicateur_model->find_all();
                $records = $this->indicateur_model->get_indicateur_by_departement('72');
	...
	*/
	...
	{
		$this->auth->restrict('Indicateur.Sarthe.Create');
		...
		if (isset($_POST['save']))
		{
			if ($insert_id = $this->save_indicateur())
			{
				...
				redirect(SITE_AREA .'/sarthe/indicateur');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (isset($_POST['save']))
		{
			$this->auth->restrict('Indicateur.Sarthe.Edit');
		...
		{
			$this->auth->restrict('Indicateur.Sarthe.Delete');
