<div class="admin-box">
	<h3><?php echo lang('adaptation_title') ?>Maine-et-Loire</h3>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Avis.Maineetloire.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<tfoot>
				<?php if ($this->auth->has_permission('Avis.Maineetloire.Delete')) : ?>
				<tr>
					<td colspan="11">
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('Avis.Maineetloire.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/maineetloire/avis/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->ID_PCET.' - '.$record->NOM_TYPE.' - '.$record->Nom_Commune.$record->Nom_Departement.$record->NOM_EPCI.$record->nom_pays.$record->nom_pnr) ?></td>
				<?php else: ?>
