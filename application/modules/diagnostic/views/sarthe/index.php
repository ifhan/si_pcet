<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Diagnostic.Sarthe.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<tfoot>
				<?php if ($this->auth->has_permission('Diagnostic.Sarthe.Delete')) : ?>
				<tr>
					<td colspan="9">
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('Diagnostic.Sarthe.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/sarthe/diagnostic/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->ID_PCET.' - '.$record->NOM_TYPE.' - '.$record->Nom_Commune.$record->Nom_Departement.$record->NOM_EPCI.$record->nom_pays.$record->nom_pnr) ?></td>
				<?php else: ?>
