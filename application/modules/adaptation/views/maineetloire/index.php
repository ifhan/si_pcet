<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Adaptation.Maineetloire.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<?php foreach ($records as $record) : ?>
				<tr>
					...
					<?php endif;?>
                                    ...
                                    <td width="20%">
                                        <?php echo anchor(SITE_AREA .'/maineetloire/adaptation/show/'. $record->id, '<i class="icon-info-sign">&nbsp;</i>Voir le volet "Adaptation"') ?><br />
                                        <?php if ($this->auth->has_permission('Adaptation.Maineetloire.Edit')) : ?>
                                            <?php echo anchor(SITE_AREA .'/maineetloire/adaptation/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>Modifier') ?><br />
                                        <?php endif; ?>     
                                    </td>
				</tr>
			<?php endforeach; ?>
