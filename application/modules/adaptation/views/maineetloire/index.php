<div class="admin-box">
	<h3><?php echo lang('adaptation_index'); ?>Maine-et-Loire</h3>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			...
			<?php foreach ($records as $record) : ?>
				...
				<td><?php echo anchor(SITE_AREA .'/maineetloire/adaptation/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>' .  $record->ID_PCET.' - '.$record->NOM_TYPE.' - '.$record->Nom_Commune.$record->Nom_Departement.$record->NOM_EPCI.$record->nom_pays.$record->nom_pnr) ?></td>
				<?php else: ?>
