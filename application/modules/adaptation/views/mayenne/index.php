<div class="admin-box">
	<h3><?php echo lang('adaptation_title'); ?>Mayenne</h3>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Adaptation.Mayenne.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<tfoot>
				<?php if ($this->auth->has_permission('Adaptation.Mayenne.Delete')) : ?>
				<tr>
					<td colspan="6">
			...
			<?php foreach ($records as $record) : ?>
				<tr>
					...
					<?php endif;?>
                                    ...
                                    <td width="20%">
                                        <?php echo anchor(SITE_AREA .'/mayenne/adaptation/show/'. $record->id, '<i class="icon-info-sign">&nbsp;</i>Voir le volet "Adaptation"') ?><br />
                                        <?php if ($this->auth->has_permission('Adaptation.Mayenne.Edit')) : ?>
                                            <?php echo anchor(SITE_AREA .'/mayenne/adaptation/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>Modifier') ?><br />
                                        <?php endif; ?>     
                                    </td>
				</tr>
			<?php endforeach; ?>
