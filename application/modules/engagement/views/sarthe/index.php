<div class="admin-box">
	<h3><?php echo lang('engagement_title'); ?>Sarthe</h3>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
                                    <?php if ($this->auth->has_permission('Engagement.Sarthe.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<?php foreach ($records as $record) : ?>
				<tr>
                                    ...
                                    <td width="20%">
                                        <?php echo anchor(SITE_AREA .'/sarthe/engagement/show/'. $record->id, '<i class="icon-info-sign">&nbsp;</i>Voir l\'engagement de la démarche') ?><br />                                        
                                        <?php if ($this->auth->has_permission('Engagement.Sarthe.Edit')) : ?>
                                            <?php echo anchor(SITE_AREA .'/sarthe/engagement/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>Modifier') ?><br />
                                        <?php endif; ?> 
                                    </td>
				</tr>
			<?php endforeach; ?>
