...
class maineetloire extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Engagement.Maineetloire.View');
	...
	*/
	public function index()
	{
		...
		$records = $this->engagement_model->get_engagement_by_departement('49');
	...
	*/
	...
	{
		$this->auth->restrict('Engagement.Maineetloire.Create');
		...
		if (isset($_POST['save']))
		{
			if ($insert_id = $this->save_engagement())
			{
				...
				redirect(SITE_AREA .'/maineetloire/engagement');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (isset($_POST['save']))
		{
			$this->auth->restrict('Engagement.Maineetloire.Edit');
		...
		{
			$this->auth->restrict('Engagement.Maineetloire.Delete');
		...
		Displays a record of Engagement data
	*/
	public function show()
	{
            $id = $this->uri->segment(5);
            $engagement = $this->engagement_model->find($id);
            $structure = $this->pcet_model->get_structure_by_id_engagement($id);
            
            Template::set('engagement', $engagement);
            Template::set('structure', $structure);
            Template::set('toolbar_title', lang('engagement_show'));
