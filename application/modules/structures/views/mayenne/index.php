<div class="admin-box">
	<h3><?php echo lang('structures_porteuses'); ?>Mayenne</h3>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Structures.Mayenne.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<?php foreach ($records as $record) : ?>
				<tr>
					...
					<td>
                                            <?php if ($this->auth->has_permission('Structures.Mayenne.Edit')) : ?>
                                                <?php echo anchor(SITE_AREA .'/mayenne/structures/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>Modifier') ?><br />
                                            <?php endif; ?>
                                            <?php echo anchor(SITE_AREA .'/mayenne/contacts/show/'. $record->ID_STR, '<i class="icon-envelope">&nbsp;</i>Voir les contacts') ?>
                                        </td>
				</tr>
			<?php endforeach; ?>
			<?php else: ?>
