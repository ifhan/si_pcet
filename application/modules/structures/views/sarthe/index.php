<div class="admin-box">
	<h3><?php echo lang('structures_porteuses'); ?>Sarthe</h3>
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Structures.Sarthe.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<?php foreach ($records as $record) : ?>
				<tr>
					...
					<td>
                                            <?php if ($this->auth->has_permission('Structures.Sarthe.Edit')) : ?>
                                                <?php echo anchor(SITE_AREA .'/sarthe/structures/edit/'. $record->id, '<i class="icon-pencil">&nbsp;</i>Modifier') ?><br />
                                            <?php endif; ?>
                                            <?php echo anchor(SITE_AREA .'/sarthe/contacts/show/'. $record->ID_STR, '<i class="icon-envelope">&nbsp;</i>Voir les contacts') ?>
                                        </td>
				</tr>
			<?php endforeach; ?>
			<?php else: ?>
