...
class sarthe extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Structures.Sarthe.View');
	...
	*/
	public function index()
	{
		...
		$records = $this->structures_model->get_structures_by_departement('72');

		Template::set('records', $records);
		Template::set('toolbar_title', 'Manage Structures');
		Template::render();
	}
	...
	*/
	...
	{
		$this->auth->restrict('Structures.Sarthe.Create');
		$communes = $this->communes_model->get_communes_by_departement('72');
		$departements = $this->departements_model->get_departement_by_insee_departement('72');
		$communautes_agglomeration = $this->epci_model->get_epci_by_departement_by_type("02472",'1');
		$communautes_urbaines = $this->epci_model->get_epci_by_departement_by_type("02472",'2');
		$communautes_communes = $this->epci_model->get_epci_by_departement_by_type("02472",'4');
		$array_pays = $this->pays_model->get_pays_by_departement('72');
		$array_pnr = $this->pnr_model->get_pnr_by_departement('72');
		...
		if (isset($_POST['save']))
		{
			if ($insert_id = $this->save_structures())
			{
				...
				redirect(SITE_AREA .'/sarthe/structures');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (isset($_POST['save']))
		{
			$this->auth->restrict('Structures.Sarthe.Edit');
		...
		{
			$this->auth->restrict('Structures.Sarthe.Delete');
